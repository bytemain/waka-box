...
const oldPrepareStackTraceDesc = Reflect.getOwnPropertyDescriptor(
  Error,
  "prepareStackTrace"
);
...
if (typeof OriginalCallSite === "function") {
  ...
  class CallSite {
    constructor(callSite) {
      Object.defineProperties(
        this,
        applyCallSiteGetters(callSite, callSiteGetters)
      );
    }
    getThis() {
      return undefined;
  ...
  (function setupCallSite() {
    for (let i = 0; i < callSiteGetters.length; i++) {
      ...
      const funcProp = Object.getOwnPropertyDescriptor(
      ...
      const newWrapped = (error, sst) => {
        if (host.Array.isArray(sst)) {
          for (let i = 0; i < sst.length; i++) {
            ...
            if (
              typeof cs === "object" &&
              local.Reflect.getPrototypeOf(cs) === OriginalCallSite.prototype
            ) {
